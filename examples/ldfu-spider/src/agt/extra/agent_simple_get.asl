...
/**
 ...
 The Get plan execute the Get external action for all referenced entrypoints. To add entrypoints, the agent must
 own beliefs in the format
 entryPointGet(uri)  // uri of the resource graph, it can be either a real uri (ie : "https://www.wikidata.org/entity/Q2814098"),
                       // or a local path (ie: "ttl/instances_sosa.ttl").

* @author Noé SAFFAF
*/

entryPointGet("https://www.wikidata.org/entity/Q2814098").

!start.

+!start : entryPointGet(IRI) <-
    .my_name(NAME);
