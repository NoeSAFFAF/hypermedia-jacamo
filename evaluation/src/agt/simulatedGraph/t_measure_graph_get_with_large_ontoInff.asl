...
entryPointGet("http://localhost:3030/simulatedGraph?graph=NodeID_1").
...
+!create_artifact_ldfu : true <-
     ...
     makeArtifact(NAME_ART,"org.hypermedea.LinkedDataFuSpider",["getGraph.n3",true],ART_ID);
     focus(ART_ID);
     .
...
+!start : true <-
	...
	register("onto/simulatedGraphLargeOntology.ttl");
...
+!profileWithCPUArtifact : true <-
     ...
     !count;
     writeEvaluationReport("agentGetGraphWithLargeOntoInff.csv");
.
...
+!count : true <-
    .count(rdf(_,"http://www.w3.org/1999/02/22-rdf-syntax-ns#type","http://www.semanticweb.org/noesaffaf/richSimulatedGraph#Node"),COUNT);
